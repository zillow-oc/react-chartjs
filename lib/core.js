      ...
      Chart.types.Doughnut.extend({
        ...
        draw: function () {
          ...
          labels.map(function (label, i) {
            this.chart.ctx.font = label.font;
            this.chart.ctx.fillStyle = label.fillStyle || '#000';
            this.chart.ctx.textBaseline = label.textBaseline || 'middle';
            var text = label.value || '';
            var textX = label.x || Math.round((this.chart.width - this.chart.ctx.measureText(text).width) / 2);
            var textY = label.y || (this.chart.height / 2.25) + (i * 20);
